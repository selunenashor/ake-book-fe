...
const deleteAuthorID = document.querySelector('input#idAuthorDelete')
const deleteAuthorConfirm = document.querySelector('button#DeleteAuthorConfirm')

deleteButton.forEach(element => {
    element.addEventListener('click', function(){
        document.querySelector('span#delAuthorName').innerHTML = element.getAttribute('author-name')
        deleteAuthorID.value = element.getAttribute('author-id')
    })
});
...
buttonAuthorConfirm.addEventListener('click', function() {
    ...
    if (name === null){
        document.querySelector('div#error_addAuthor').innerHTML = "Tên là bắt buộc"
        return
    }
    ...
    .then(response => {
        if (!response.ok) {
            document.querySelector('div#error_addAuthor').innerHTML = "Có lỗi xảy ra!"
            return
        }
        else{
...
deleteAuthorConfirm.addEventListener('click', function(){
    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    fetch('/admin/author/delete/' + deleteAuthorID.value, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': csrfToken
    ...
    .then(response => {
        if (!response.ok) {
            document.querySelector('div#error_delAuthor').innerHTML = "Có lỗi xảy ra!"
            return
        }
        else{
            window.location.reload()
